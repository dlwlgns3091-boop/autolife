...
from ..models import MonthEndTask
...
def list_items(db: Session = Depends(get_db)):
    return db.query(MonthEndTask).order_by(MonthEndTask.order, MonthEndTask.id).all()
...
def create_item(data: ChecklistItemCreate, db: Session = Depends(get_db)):
    item = MonthEndTask(**data.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
    return item
...
def update_item(item_id: int, data: ChecklistItemUpdate, db: Session = Depends(get_db)):
    item = db.get(MonthEndTask, item_id)
    if not item:
...
def reset_month_end(db: Session = Depends(get_db)):
    db.query(MonthEndTask).update({"is_checked": False})
    db.commit()
