...
def list_items(db: Session = Depends(get_db)):
    return db.query(WeeklyTask).order_by(WeeklyTask.order, WeeklyTask.id).all()
...
def reset_weekly(db: Session = Depends(get_db)):
    db.query(WeeklyTask).update({"is_checked": False})
    db.commit()
